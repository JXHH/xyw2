...
function Dsy(){
	...
	var s=["province","city"];//两个select的name
	var opt0 = ["省份","市"];//初始值
...
<h1>添加攻略
	...
	<label>
 	<span>上传封面图:</span>	    	<input type="file" name="file" id="file"></input>
		...
		<label><span>省份:</span> <select id="province"></select> </label>
               <label><span>市:</span>  <select id="city" ></select></label>
    ...
    <script type="text/javascript">
        ...
        editor.create();
        function addtt(){
            var ttcity = document.getElementById("city").value;
    		...
    		formdata.append ("ttcontent" , ttcontent);
    		 $.ajax({
    	            ...
    	            error:function(){
    	            	alert("添加攻略成功~");
     	            	window.location.href="/xyw2/user/traveltip";
    	            },
